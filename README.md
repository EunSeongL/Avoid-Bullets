...
### 기술적 회고
* **성과**: 클럭 트리부터 메모리 기반 객체 관리까지 임베디드 시스템의 전체 동작 흐름을 직접 제어하며 펌웨어 설계 역량 확보
* **메모리 최적화**: STM32F103의 제한된 20KB RAM 환경에서 구조체 배열과 스택 메모리 관리의 중요성 체감
* **개선 방향**: 이미지 데이터를 플래시 메모리에 저장하고 버퍼로 불러오는 구조를 적용했다면, 비트맵 기반의 더 화려한 그래픽 구현이 가능했을 것으로 분석됨
